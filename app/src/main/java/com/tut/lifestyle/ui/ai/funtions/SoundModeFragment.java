...
public class SoundModeFragment extends Fragment implements RemoteRepresentationListener {

    public static final String TAG = "SoundModeFragment";
    public static final String TITLE = "Sound Mode";
    View root;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        root = inflater.inflate(R.layout.fragment_sound_mode, container, false);
        setOnClickListeners();
        return root;
    }
    ...
    private void setOnClickListeners(){
        root.findViewById(R.id.option1).setOnClickListener(v -> {
            D2SManager.getInstance().sendRemoteRepresentation(OCFAttributes.soundMode, ((RadioButton)v).getText().toString());
        });
        ...
        root.findViewById(R.id.option5).setOnClickListener(v -> {
            D2SManager.getInstance().sendRemoteRepresentation(OCFAttributes.soundMode, ((RadioButton)v).getText().toString());
        });
    }
    ...
    public void onRepresentationReceived(OCFResponse response) {
        if(response.getStatus() == OCFConstants.RESULT_OK){
            if(response.getAttr().equalsIgnoreCase(OCFAttributes.soundMode)){
                // dismiss loading
